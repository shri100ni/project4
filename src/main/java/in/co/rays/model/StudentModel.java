	...
	public StudentBean findByPK(long pk) throws ApplicationException {
		...
		try {
			...
			while (rs.next()) {
				...
				bean.setDateOfBirth(rs.getDate(6));
...
//		}
		...
		try {
			...
			pstmt.executeUpdate();
			conn.commit();
			System.out.println("studend Add==>>>8888");
	...
	public List search(StudentBean bean, int pageNo, int pageSize) throws ApplicationException {
		...
		if (bean != null) {
			...
			if (bean.getDateOfBirth() != null && bean.getDateOfBirth().getDate() > 0) {
				sql.append(" AND DOB = " + bean.getDateOfBirth());
			}
